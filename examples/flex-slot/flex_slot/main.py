...
def ifnotpurchasedalready(project_id, dataset_id):
    ...
    sql =
    """select value from {0}.{1}.flex_purchase_update_indicator where action = 'purchase""".format(project_id, dataset_id)
    query_job = client.query(sql)
    for row in query_job:
        purchase_ind = row["value"]
    sql_update = """select value from {0}.{1}.flex_purchase_update_indicator where action = 'update'""".format(project_id,dataset id)
    # print (sqL)
    ...
    try:
        ...
        if float(slot_consumed) > threshold:
            ...
            if ifnotpurchasedalready(project_id, dataset_id) :
                flex_slot_purchase_main(config, bucket _id, Logger)
